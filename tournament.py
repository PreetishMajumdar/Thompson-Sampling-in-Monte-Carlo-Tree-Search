...
def random_rollout(state, max_depth=30):
    """
    Phase 3: Simulation (Optimized for High-Dimensional Boards).
    Plays random moves up to a max_depth. If no terminal state is reached,
    uses a material-count heuristic to predict the winner instantly.
    """
    current_state = state
    moves_made = 0
    
    while not current_state.is_terminal() and moves_made < max_depth:
    ...
    # Heuristic Fallback: Count total stones owned by each player
    p1_count = np.sum(current_state.board == 1)
    p2_count = np.sum(current_state.board == 2)
    ...
    if p1_count > p2_count:
        ...
        return None # Evaluated as a Draw
...
# =====================================================================
# 3. TOURNAMENT RUNNER (WITH INCREMENTAL CSV SAVING)
# =====================================================================

def run_tournament(game_class, board_size, num_games, iterations, csv_filename):
    file_exists = os.path.isfile(csv_filename)
    
    print(f"==================================================")
    print(f"LAUNCHING HPC TOURNAMENT")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run MCTS Tournament on CSF.")
    parser.add_argument("--game", type=str, choices=["hex", "gomoku"], required=True, help="Which game to play.")
    parser.add_argument("--games", type=int, default=500, help="Total number of games to play.")
    ...
    if args.game == "hex":
        ...
        out_csv = f"csf_hex_{b_size}x{b_size}_results.csv"
    else:
        target_class = GomokuState
        b_size = args.board_size if args.board_size else 15
        out_csv = f"csf_gomoku_{b_size}x{b_size}_results.csv"
