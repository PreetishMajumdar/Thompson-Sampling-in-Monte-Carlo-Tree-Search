...
def random_rollout(state, max_depth=None):
    current_state = state
    depth = 0
...
def run_mcts(root_state, iterations, selection_strategy, **strategy_kwargs):
    ...
    for _ in range(iterations):
        ...
        # You can pass max_depth=60 here if Gomoku proves too slow on the CSF
...
def run_tournament_chunk(game_class, board_size, start_id, chunk_size, iterations, csv_filename):
    ...
    with open(csv_filename, mode='a', newline='') as f:
        ...
        for game_id in range(start_id, start_id + chunk_size):
            # Alternate opening turns for fairness
            if game_id % 2 != 0:
...
if __name__ == "__main__":
    ...
    if args.game == "hex":
        ...
        out_csv = f"results/csf_hex_{b_size}x{b_size}_results_chunk{args.start_id}.csv"
    else:
        target_class = GomokuState
        b_size = args.board_size if args.board_size else 15
        out_csv = f"results/csf_gomoku_{b_size}x{b_size}_results_chunk{args.start_id}.csv"
